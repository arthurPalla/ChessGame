...
int main(int argc, char const *argv[])
{
  ...
  while (!WindowShouldClose()) {
    ...
    if (IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
      ...
      if (selected == NULL) {
        get_piece_atco((int)((mouse_pos.x * 8) / WIDTH),
                       (int)((mouse_pos.y * 8) / HEIGHT), game, &selected);
        if (selected != NULL) {
        ...
        if (!roque(selected, game,
                   (int)((mouse_pos.x * 8) / WIDTH),
                   (int)((mouse_pos.y * 8) / HEIGHT))
            && !move_piece_to(selected, game, (int)((mouse_pos.x * 8) / WIDTH),
                              (int)((mouse_pos.y * 8) / HEIGHT))) {
          if (!echec_color(game, (*selected).col)) {
