...
void highlight(piece p, bool flag, Color col)
{
  ...
  if (flag) {
    ...
    if ((p.y % 2 == 0 && p.x % 2 == 0) || (p.y % 2 == 1 && p.x % 2 == 1)) {
      DrawRectangle(p.x * WIDTH / 8, p.y * HEIGHT / 8, WIDTH / 8, WIDTH / 8,
                    GRAY);
    } else {
      DrawRectangle(p.x * WIDTH / 8, p.y * HEIGHT / 8, WIDTH / 8, WIDTH / 8,
                    RAYWHITE);
...
void drawgame(piece * jeu)
{
  ...
  for (int i = 0; i < 64; i++) {
    if (jeu[i].type_piece != NONE) {
      DrawTexture(jeu[i].sprite, jeu[i].x * WIDTH / 8, jeu[i].y * HEIGHT / 8,
                  WHITE);
    }
...
bool roque(piece * p, piece * game, int x, int y)
{
  ...
  if ((y % 2 == 0 && x % 2 == 0) || (y % 2 == 1 && x % 2 == 1)) {
    ...
    DrawRectangle(x * WIDTH / 8, y * HEIGHT / 8, WIDTH / 8, WIDTH / 8,
                  RAYWHITE);
  }
  EndDrawing();
  piece *tower = NULL;
  ...
  DrawTexture((*tower).sprite, (*tower).x * WIDTH / 8, (*tower).y * HEIGHT / 8,
              WHITE);
  EndDrawing();
  return true;
...
{
  if (can_move((*p), game, x, y, true) == false) {
    return false;
  }
  BeginDrawing();
  if (((*p).y % 2 == 0 && (*p).x % 2 == 0)
